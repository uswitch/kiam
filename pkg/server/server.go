...
func NewServer(config *Config) (*KiamServer, error) {
	...
	server.assumePolicy = Policies(NewRequestingAnnotatedRolePolicy(server.pods, arnResolver), NewNamespacePermittedRoleNamePolicy(server.namespaces, server.pods, arnResolver))
